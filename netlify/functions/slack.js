...
exports.handler = async (event) => {
  ...
  // Action: list channels (public + private)
  if (action === "list_channels") {
    try {
      var channels = [];
      var cursor = "";
      do {
        var url = "https://slack.com/api/conversations.list?types=public_channel,private_channel&limit=200";
        if (cursor) url += "&cursor=" + encodeURIComponent(cursor);
        var r = await fetch(url, { headers: { "Authorization": "Bearer " + token } });
        var j = await r.json();
        if (!j.ok) return resp(500, { error: "Slack API error", details: j.error });
        channels = channels.concat(j.channels.map(function(c) {
          return { id: c.id, name: c.name, is_private: c.is_private };
        }));
  ...
  if (action === "send") {
    ...
    try {
      // Only attempt to join public channels — private channels require manual bot invite
      var infoR = await fetch("https://slack.com/api/conversations.info?channel=" + channel, {
        headers: { "Authorization": "Bearer " + token }
      });
      var infoJ = await infoR.json();
      var isPrivate = infoJ.ok && infoJ.channel && infoJ.channel.is_private;
